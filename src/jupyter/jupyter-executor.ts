...
export class VSCodeJupyterExecutor implements JupyterExecutor {
  ...
  /**
   * Helper: Create a new Jupyter notebook
   */
  static async createNotebook(filename?: string): Promise<string> {
    // Create a new notebook document
    const notebookData = new vscode.NotebookData([
      new vscode.NotebookCellData(
        vscode.NotebookCellKind.Code,
        '# Causal Inference Analysis\n# Generated by Causal Inference Assistant\n',
        'python'
      ),
    ]);
  ...
  /**
   * Helper: Create notebook, add code, execute, and return results
   * This is a convenience method for testing and quick execution
   */
  static async executeInNewNotebook(code: string): Promise<ExecutionResult> {
    // Create new notebook
    const notebookUri = await VSCodeJupyterExecutor.createNotebook();
    ...
    // Don't disconnect - leave notebook open for user to see
    return result;
  }

  /**
   * Helper: Execute code in active notebook or create new one
   */
  static async executeCode(code: string): Promise<ExecutionResult> {
    // Try to get active notebook first
    let notebookUri = await VSCodeJupyterExecutor.getActiveNotebook();

    // If no active notebook, create one
    if (!notebookUri) {
      notebookUri = await VSCodeJupyterExecutor.createNotebook();
    }
